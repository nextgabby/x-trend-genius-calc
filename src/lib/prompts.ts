...
export function buildKeywordAnalysisPrompt(
  ...
  const exactKeywordsInstruction = useExactKeywords
    ? `\n\nCRITICAL — EXACT KEYWORDS MODE (STRICT): The user has provided a client-approved search query that MUST keep the SAME keywords and the SAME boolean intent. Rules:
- PRESERVE the user's AND / OR / parentheses intent. Do NOT flatten into all ORs. Example: "(A OR B) AND (C OR D)" must stay an AND of two OR-groups — never become "A OR B OR C OR D".
...
5. OPPONENT/MATCHUP TERMS — If this is a competition, you MUST include "Team A vs Team B" pairings for confirmed opponents. This is NOT optional when the campaign dates overlap with a known competition. Use your knowledge of tournament draws, fixtures, and schedules to identify the specific opponents. For example, if the user enters a national soccer team with World Cup 2026 dates, you KNOW the group stage draw — include the matchup terms. Only add a queryWarning about unconfirmed matchups if the competition bracket genuinely has not been determined yet (e.g., knockout round opponents that depend on group results). Group stage opponents from completed draws are KNOWN FACTS, not uncertainties.
6. EVENT-SPECIFIC TERMS — Year-specific terms, host city names, tournament phases, venue names that are relevant to the campaign window (e.g., "World Cup 2026", "BMO Field", "group stage"). Only include event-level terms if they are COMBINED with the user's specific topic. "World Cup 2026" alone matches ALL World Cup conversation — if the campaign is about Team Canada, do not include generic tournament-wide terms. Instead, prefer terms that tie the event to the topic (e.g., "Canada World Cup" over "World Cup 2026"). This also applies to generic event hashtags — #WC2026, #SuperBowl, #Olympics, #WorldSeries, #MarchMadness, etc. all match conversation about every team and participant, not just the user's topic. Only include event-level hashtags if they are specific to the user's niche (e.g., #CanMNT over #WC2026, #ChiefsKingdom over #SuperBowl, #TeamUSA over #Olympics).
...
- Identify any opponent names, matchup pairings, or fixture details mentioned in the tweets — these should be included in category 5 of the keyword checklist
